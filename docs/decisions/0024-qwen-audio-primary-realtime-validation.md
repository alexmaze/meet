...
- 状态：部分由 ADR-0025、ADR-0026 修订
- 日期：2026-08-09
- 修订：ADR-0001、ADR-0023
- 验证修订：2026-08-09，依据真实 WebRTC 上行排障结果修正音轨挂载时序

说明：第 8 项关于真实设备测试与默认模型确定时点的决定，以及“完成真实设备测试后决定默认生产模型”的后果，已由 ADR-0025 修订。第一版现在默认使用 `qwen-audio-3.0-realtime-plus` 开展业务开发。ADR-0026 将默认传输改为服务端 WebSocket PCM 中继；本决策中的 WebRTC 音轨时序保留为实验路径，模型能力、`smart_turn`、安全边界和对照方案继续有效。
