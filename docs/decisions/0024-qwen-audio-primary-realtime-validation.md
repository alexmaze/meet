...
9. WebRTC 由服务端创建名为 `txt` 的 DataChannel。客户端从该通道收到 `session.created` 后，通过同一通道发送 `session.update`；收到 `session.updated` 后才恢复麦克风音轨并进入活动状态，确保首次发送音频前已经配置 `smart_turn`。后续控制事件与供应商事件也复用 `txt` 通道。
