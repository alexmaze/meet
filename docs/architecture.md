...
`PersonaDefinition` 至少包含背景、核心性格、与用户关系、说话习惯、情绪风格、对话目标和示例台词。系统根据这些字段生成运行时提示词，但结构化字段始终是可查看、可编辑的事实来源。
...
6. 已实现摘要、长期记忆、`pg-boss` Worker、分级记忆审阅与后续通话上下文注入；下一步实现 MediaStore 和录音生命周期；
