...
角色卡迁移与私人关系迁移使用两个独立协议。`meet-character-relationship-v1` 只允许当前已认证账号按一个可见角色导出自己的通话、消息、最终摘要（缺失时回退到最新成功的增量检查点）和全部长期记忆状态。包不包含账号 ID、角色定义、模型配置 ID、Provider 事件、运行时快照、Mem0 外部记录、图片或录音，并带版本、25 MB 上限和 SHA-256 完整性校验。

关系包导入目标由当前账号在角色详情中选择；内置角色优先用 `systemKey` 匹配，不一致时要求显式确认。服务端在一个事务中为会话、消息和记忆生成新 ID并重写账号与角色归属，来源活动通话转为已完成历史。`relationship_transfer_imports` 以目标账号和 `transferId` 阻止重复提交或文件替换，`relationship_transfer_conversation_origins` 以目标账号、目标角色和来源会话 UUID 去重，并在已导入会话删除后保留 tombstone。记忆继续使用规范化内容指纹去重，目标端现有审阅状态优先；新导入的 active 记忆通过既有索引任务同步。详细边界见 [ADR-0036](decisions/0036-account-character-relationship-data-transfer.md)。
...
ADR-0038 使用独立 `conversation_controls` 与操作回执表保存写入者、代次、连接租约、结束意图和有效连接时长，不把业务 `active` 等同于连接在线。准备连接后，WebSocket 握手在装配教学状态和连接供应商前占用该会话；浏览器 Pong 才能触发每 10 秒的认证复查与服务端续租，租期 45 秒。中继对每个音频与控制事件检查本地截止时间，阻止进程停顿后过期帧继续转发；数据库对消息追加、结束、心跳和释放再检查原子写入资格。断开只释放连接，保留该代次的收尾资格，另一次合法准备递增代次后旧写入者失效。

结束请求以固定请求 ID 和目标序号进行幂等提交和结果查询。完成状态与工作项投递仍在同一业务事务中；回顾和记忆状态直接来自已有工作项及来源数据。普通/临时会话恢复延用运行时快照，新的已完成会话续聊按当前角色配置创建。浏览器只持久化按部署和账号隔离的最小结束意图，不缓存正文或音频；待同步消息在通话组件内存中保留，历史覆盖层不卸载该组件。

首页通过本人会话概览和独立收藏接口组合，不读取被监护账号的回访资料。`character_favorites` 以账号与角色为联合主键，写入时重新校验可见性；记忆读取在账号权限约束内组合角色和来源会话条件，并在分页前过滤。
...
6. 已实现最终摘要、长通话增量摘要检查点、长期记忆、`pg-boss` Worker、分级记忆审阅、后续通话上下文注入，以及 MediaStore、媒体元数据、受权读取/保留/删除和临时媒体清理基础；已按 ADR-0037 接入单连接 28 分钟预告、20 个有效用户轮次上下文刷新，以及等待双端空闲、音频耗尽与字幕持久化后的主动续接，继续复用同一业务会话与不可变快照；下一步完成实机长通话验收，以及按次录音采集与历史回放；
