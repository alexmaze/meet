...
通话结束后显示文字保存、结束确认、回顾和记忆处理的独立状态，保存失败可重试。再次打开可从首页或本人历史继续未结束通话，保留原模式和角色配置；另一有效设备正在使用时不会强制接管。首页同时提供最近角色和账号独立收藏，角色详情展示上次普通通话回顾。浏览器不持久保存私人正文，系统杀页可能丢失尚未同步的尾部，见 [ADR-0038](docs/decisions/0038-conversation-continuity.md)。

本次升级包含 `0024_conversation_continuity.sql`，需要执行 `pnpm db:migrate`。新增写入资格校验后，旧前端请求不再受理；请在现有通话全部收尾后升级 API/Worker，并刷新浏览器或 PWA 使用对应前端版本。
