...
角色创建与编辑页会展示 `qwen-audio-3.0-realtime-plus` 当前 5 个系统音色：`longanqian`、`longanlingxin`、`longanlingxi`、`longanxiaoxin`、`longanlufeng`。管理员和成人可以用固定短句生成并播放试听；试听由带账号认证的 API 在服务端连接千问、汇总 24 kHz PCM 并返回短 WAV，不创建业务会话、不申请麦克风，也不向浏览器暴露 API Key 或 Endpoint。
