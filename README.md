...
1. 在现有 `pg-boss` Worker 上实现 MediaStore、按次录音、临时音频过期与保留生命周期，并保持业务层不直接拼接存储路径。
2. 继续将当前千问链路收敛到 Provider Adapter 边界，并为后续图像能力与供应商对照保留清晰接口。
3. 在基础角色闭环稳定后补充 AI 快速创建、头像上传与生成、角色卡导入导出和音色试听。
